...
- `GET /api/catalog/products/{id}/price`: Returns the active price (discounted) for a specific product.
- `GET /api/catalog/discounts/preview/{productId}`: Returns price preview with discounts applied for a specific product.
...
- `POST /api/admin/discounts`: Creates a new discount (requires `ROLE_ADMIN` role).
